...
export const useCashApp = () => {
  ...
  const useLoopStorage = (sotrageKey, fallbackState) =>  {
    const [value, setValue] = useState(
      JSON.parse(localStorage.getItem(sotrageKey)) ?? fallbackState
    )
    useEffect(()=> {
      localStorage.setItem(sotrageKey, JSON.stringify(value));
    }, [value, setValue])
  ...
  const makeTransaction = async (fromWallet, toWallet, amount, reference) => {
    ...
    // Create instruction to send SOL from owner to recepient
  ...
  // CReate the function to RUN the transaction. This will added to button
   const doTransaction = async ({ amount, receiver, transactionPurpose }) => {
      ...
      // Craete transaction history object
